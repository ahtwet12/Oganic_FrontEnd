...
export class ProductService {
  ...
  // getProductCountByCategoryId(categoryId: number): Observable<number> {
  //   return this.http.get<number>(`PRODUCT_API/product-count?categoryId=${categoryId}`);
  // }
  ...
  createProduct(name:string,description: string,price: string,quantity:number,categoryId: number,imageIds: Array<string>, date:string):Observable<any>{
    return this.http.post(PRODUCT_API +'create',{name,description,price,quantity,categoryId,imageIds},httpOptions);
  }

  updateProduct(id: number,name:string,description: string,price: string,quantity:number,categoryId: number,imageIds: Array<string>, update:string):Observable<any>{
