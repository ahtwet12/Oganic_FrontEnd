...
export class ProductComponent implements OnInit {
  ...
  openUpdate(data : any){
    this.listImageChoosen = [];
      ...
      this.productForm.date = new Date(data.date).toLocaleString('en-GB', { hour12: false });
      this.productForm.updatedate = new Date(data.updatedate).toLocaleString('en-GB', { hour12: false });
  ...
  createProduct(){
    ...
    const {name,description,price,quantity,categoryId,imageIds,date} = this.productForm;
    const formattedDate = new Date().toLocaleString();

    console.log(this.productForm);
    this.productService.createProduct(name,description,price,quantity,categoryId,imageIds,date).subscribe({
      next: res =>{
        this.getListProduct();
        this.showForm = false;
  ...
  updateProduct(){
    ...
    const {id,name,description,price,quantity,categoryId,imageIds, updatedate} = this.productForm;
    const formattedDate = new Date().toLocaleString();

    console.log(this.productForm);
    this.productService.updateProduct(id,name,description,price,quantity,categoryId,imageIds,updatedate).subscribe({
      next: res =>{
        this.getListProduct();
        this.showForm = false;
